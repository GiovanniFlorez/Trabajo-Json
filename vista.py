...
class Vista:
    ...
    def mostrar_menu(self, callback):
        ...
        texto1 = tk.Label(self.contenedor, text="Seleccione una opción:")
        texto1.pack()
    ...
    def obtener_datos(self, callback_volver):
        ...
        def guardar_datos():
            ...
            # Call the callback for saving data
            callback_volver(datos)
        
        boton_guardar = tk.Button(self.contenedor, text="Guardar", command=guardar_datos)
        boton_guardar.pack()
        
        boton_volver = tk.Button(self.contenedor, text="Volver", command=lambda: callback_volver(None))
        boton_volver.pack()
    ...
    def obtener_indice(self, callback, callback_volver):
        ...
        indice_label = tk.Label(self.contenedor, text="Índice")
        indice_label.pack()
        ...
        boton_volver = tk.Button(self.contenedor, text="Volver", command=lambda: callback_volver())
        boton_volver.pack()
    ...
    def actualizar_datos(self, index, modelo, callback_volver):
        ...
        def guardar_cambios():
            ...
            self.mostrar_exito("Éxito", "Registro actualizado exitosamente.")
            self.mostrar_datos(modelo.leer())
            self.limpiar_contenedor()  # Limpia los campos
            callback_volver()

        boton_guardar = tk.Button(self.contenedor, text="Guardar", command=guardar_cambios)
        boton_guardar.pack()

        boton_volver = tk.Button(self.contenedor, text="Volver", command=callback_volver)
        boton_volver.pack()
